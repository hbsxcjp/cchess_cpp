...
void ShadowWindowLine(char *str)
{
	...
	posText.X = rc.Left + 2;
	posText.Y = rc.Top + 2;
	WriteConsoleOutputCharacterA(hOut, str, strlen(str), posText, NULL);
	//DrawBox(true, rc);
	DrawBox(false, rc);
...
void DrawBox(bool bSingle, SMALL_RECT rc) // 函数功能：画边框
{
	...
	{	
		pos.Y = rc.Top;
		// 画上边界
		WriteConsoleOutputCharacterA(hOut, &chBox[4], 1, pos, NULL);
		// 画左上角
		if(pos.X == rc.Left+1)
		{
			pos.X--;
			WriteConsoleOutputCharacterA(hOut, &chBox[0],1, pos, NULL);
			pos.X++;
		}
		// 画右上角
		if(pos.X == rc.Right-2)
		{
			pos.X++;
			WriteConsoleOutputCharacterA(hOut, &chBox[1], 1, pos, NULL);
			pos.X--;
		}
		pos.Y = rc.Bottom;
		// 画下边界
		...
		{
			pos.X--;
			WriteConsoleOutputCharacterA(hOut, &chBox[2], 1, pos, NULL);
			pos.X++;
		}
		// 画右下角
		if(pos.X==rc.Right-2)
		{
			pos.X++;
			WriteConsoleOutputCharacterA(hOut, &chBox[3], 1, pos, NULL);
			pos.X--;
		}
	}
	// 画边框的左右边界
	for (pos.Y = rc.Top+1; pos.Y<=rc.Bottom-1; pos.Y++)
	{
		pos.X = rc.Left;
		// 画左边界
		WriteConsoleOutputCharacterA(hOut, &chBox[5], 1, pos, NULL);
		pos.X = rc.Right-1;
		// 画右边界
		WriteConsoleOutputCharacterA(hOut, &chBox[5], 1, pos, NULL);
	}
...
void ConsoleSpace::view()
{
	...
	GetConsoleScreenBufferInfo(hOut, &bInfo ); // 获取窗口缓冲区信息
	GetConsoleTitleA(strTitle, 255); // 获取窗口标题
	printf("当前窗口标题是：%s\n", strTitle);
	_getch();
	SetConsoleTitleA("控制台窗口操作"); // 设置窗口标题
